...
| `forceMount` | `boolean` | `false` | Force render in production. DevTools renders nothing when `NODE_ENV=production` unless this is `true`. |
...
DevTools automatically renders nothing when `NODE_ENV` is `"production"`. No conditional wrappers needed — just leave the component in your tree and it will be invisible in production builds.

To force it on in production (e.g. for a demo site), pass `forceMount`:

```tsx
<BlockTreeDevTools blocks={blocks} events={events} onClearEvents={clearEvents} forceMount />
```
