...
As of v2.0.0, the library is split into three packages:

| Package | Description |
|---------|-------------|
| [`dnd-block-tree`](https://www.npmjs.com/package/dnd-block-tree) | Full React library (re-exports `@dnd-block-tree/react`) |
...
Most users should install `dnd-block-tree`. Use `@dnd-block-tree/core` directly for non-React use cases (server-side manipulation, testing, other frameworks).
