...
Install `@dnd-block-tree/react` along with its peer dependencies:
...
No peer dependencies required for core-only usage. See the [Core Overview](/docs/core) for the full headless API.

### Peer Dependencies

When using `@dnd-block-tree/react`:
