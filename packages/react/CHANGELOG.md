...
- **Monorepo rewrite** — the library is now split into two packages:
...
- The old `dnd-block-tree` compat wrapper has been removed. Import from `@dnd-block-tree/react` instead.
...
- Publish workflow now publishes both `@dnd-block-tree/core` and `@dnd-block-tree/react`
...
- **Resizable panel** — Drag any edge or corner to resize the DevTools card (min 280x200). Toggling the diff view auto-adjusts width.
