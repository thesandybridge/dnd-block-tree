...
function makeDOMRect(top: number, left: number, width: number, height: number): DOMRect {
  return {
    top,
    left,
    width,
...
function makeDroppable(id: string, rect: DOMRect) {
  const el = {
...
describe('adaptCollisionDetection (svelte)', () => {
  it('converts droppables to candidates and returns mapped results', () => {
    ...
    expect(coreDetector).toHaveBeenCalledWith(
      ...
      { top: 25, left: 50, width: 1, height: 1, right: 51, bottom: 26 }
    )
