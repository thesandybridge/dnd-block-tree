<script lang="ts">
  import { BlockTree, generateId } from '@dnd-block-tree/svelte'
  ...
  // Determine if extension is a code file, image, or generic
...
{#snippet renderBlock({ block, isDragging, depth, isExpanded, onToggleExpand, children }: { block: FileSystemBlock, isDragging: boolean, depth: number, isExpanded: boolean, onToggleExpand: (() => void) | null, children: Snippet | null })}
  {#if block.type === 'folder'}
    ...
    >
      <div
        class="rounded-lg border border-border/30 bg-card/50 w-full min-w-0 max-w-full transition-all duration-200"
  ...
  {:else}
    ...
    >
      <div
        class="flex items-center gap-2 p-2 rounded-md w-full min-w-0 max-w-full block-item group transition-all duration-150"
        class:opacity-40={isDragging}
        class:scale-[0.98]={isDragging}
      >
        <svg class="h-4 w-4 shrink-0 text-muted-foreground cursor-grab drag-handle" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="5" r="1"/><circle cx="9" cy="12" r="1"/><circle cx="9" cy="19" r="1"/><circle cx="15" cy="5" r="1"/><circle cx="15" cy="12" r="1"/><circle cx="15" cy="19" r="1"/></svg>

        <!-- File icon (colored by extension category) -->
